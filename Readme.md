...
- **언어 전환 애니메이션**: 언어 토글도 같은 View Transitions API를 사용하지만, 페이지 전체 콘텐츠가 바뀌는 특성에 맞춰 원형 확산 대신 부드러운 크로스페이드(0.42s)를 적용했습니다. `html.vt-theme`/`html.vt-lang` 클래스로 두 전환의 애니메이션 규칙을 분리해 서로 간섭하지 않도록 했습니다. 마찬가지로 미지원 브라우저·`prefers-reduced-motion`에서는 즉시 전환됩니다.
